...
```
That will drop all tables (including ones that are not created by Kimai) in this database and re-create it, before importing the data from the `mysql` database at `127.0.0.1` on port `3306` authenticating the user `kimai` with the password `test` for import.
